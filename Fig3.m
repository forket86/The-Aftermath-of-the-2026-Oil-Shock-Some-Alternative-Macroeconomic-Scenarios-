% PACKAGE NOTE: Run from the package root with a licensed futures calibration CSV.
% PACKAGE NOTE: Set AleDirectory=1 on a Mac, or adjust the Dynare path below.
...
cases(1).use_exact_futures_path = 1;


cases(2).p_perm = 0.15;    % long-run temporarily elevated oil level: +10%
cases(2).rho_g_path = 0.55;
cases(2).use_exact_futures_path = 0;
...
p_eomarch_senate = wtifutures_change.mean_wtifutures_change(1)/100;    % EO March oil level
p_eoq2_senate = wtifutures_change.mean_wtifutures_change(2)/100; % EO Q2 oil level
...
caselab = {sprintf('Oil price returns to steady state'), ...
    sprintf('Oil price permanently elevated at +%.0f%%', cases(2).p_perm*100)};

if do_senate_balks == 1
    caselab = [caselab, {sprintf('War continues: delayed-learning scenario')}];
end
...
for i = 1:numel(cases)
    ...
    if cases(i).use_exact_futures_path==0
...
%
% Realized path:
%   Q1-Q4 2026: po = p_eomarch_senate
    ...
    for h = 1:H
        ...
        if h <= N_senate_high
            ...
            if h == 1
                ...
                po_expected(1) = p_eomarch_senate;

                % Q1 information set:
                % agents correctly expect oil to decline to the 
                % observed Q2 level. Reversion toward the new steady state starts
                % only from Q3 in their expectations.
                %po_expected(2) = p_eomarch_senate;
                po_expected(2) = p_eoq2_senate;


                for tt = 3:T_return_start
                    po_expected(tt) = p_newss_senate ...
                        + rho_po^(tt-2) * (p_eomarch_senate - p_newss_senate);
                end


            elseif h == 2
                % Q1, Q3-Q4 2026: current realized oil is at EO March level.
                po_expected(1) = p_eoq2_senate;

                % Q2 information set:
                % no surprise: oil is at Q2 levels as expected. Agents expect
                % continued reversion starting in Q3.

                for tt = 2:T_return_start
                    po_expected(tt) = p_newss_senate ...
                        + rho_po^(tt-1) * (p_eoq2_senate - p_newss_senate);
                end
                ...
                for tt = 2:T_return_start
                    po_expected(tt) = p_newss_senate ...
                        + rho_po^(tt-1) * (p_eomarch_senate - p_newss_senate);
                end
...
% Save output numbers (fig3)
%towrite = table(plotdata_2x3{2, 1:2}, 'VariableNames', {'Returns to SS', 'Permanently elevated'});
%towrite.Date = datelabels';
%towrite = movevars(towrite, "Date", "before", "Returns to SS");
%writetable(towrite, ...
%           'fig3_output.csv', 'WriteVariableNames', true)
...
%legend(l1, 'Futures price change', 'Location', 'southoutside', 'box', 'off')
